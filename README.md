...
| `--stall-patience N` | Stop annealing early if the acceptance ratio stays below `--stall-ratio` for `N` consecutive windows (default 0 = disabled). 3–5 is a good starting value when using `--routing-time`. Avoids burning the full time budget once the optimiser is clearly stuck. |
| `--stall-ratio R` | Accept-ratio threshold for `--stall-patience` (default 0.02). Only active when `--stall-patience > 0`. |
| `--flat-window N` | Stop annealing early if the energy does not change by more than 1e-6 over `N` consecutive iterations (default 0 = disabled). Useful when routing is already near-optimal and every reroute produces the same result — avoids confirming nothing improved over the full budget. 20–50 is a good starting value. |
...
| `--greedy-order {short,long,shuffle}` | Order for the initial greedy routing pass. `short` (default): shortest connections first. `long`: longest first — gives hard long-distance connections first pick of the empty grid. `shuffle`: random order per run/cycle — useful with `--runs`/`--cycles` to start the annealer from genuinely different configurations. |
...
before each cycle's placement pass (or before each `--place-runs` run). Without
it all cycles/runs start from the as-designed layout and the annealer explores
nearby configurations; `--scatter` gives each attempt a completely fresh random
starting layout so the annealer can explore basins of attraction it would
otherwise never reach. The trade-off is that a scattered start is far from
feasible — pair it with a generous `--place-time` or `--place-iters` budget so
the annealer has room to recover. At the end of a `--cycles` run a ranked summary
table is printed showing each cycle's result in energy order, with the winner
marked ★.
...
### Saving the routed result

After a run completes, a **Save As…** button appears in the results area. Click it to
write the current best board to a chosen path via a file dialog — useful when you
...
heat map (blue = low cost, red = high). Footprints are coloured by the sum of their
ratsnest length and overlap contributions; ratsnest connections are coloured by their
