...
class _MetricsPanel(ttk.Frame):
    ...
    def update(self, ev: Progress) -> None:
        ...
        if ev.budget > 0:
            ...
            self._vars["iter"].set(
                f"{ev.it} iters  {remaining:.0f}s left  ({ev.elapsed:.0f}s elapsed)")
        else:
            total = ev.total or 1
            pct = min(100, int(100 * ev.it / total))
            elapsed_str = f"  ({ev.elapsed:.0f}s)" if ev.elapsed > 0 else ""
            self._vars["iter"].set(f"{ev.it} / {ev.total}{elapsed_str}")
