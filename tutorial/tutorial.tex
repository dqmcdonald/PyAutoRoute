...
% thin accent rule used as section underline (pre-defined to avoid nested [] in optional arg)
\newcommand{\sectionrule}{\vspace{2pt}{\color{accent!35}\titlerule[0.5pt]}\vspace{2pt}}

% Section: bold sans-serif in accent colour, thin rule beneath
\titleformat{\section}
  {\color{accent}\sffamily\Large\bfseries}
  {\thesection.}{0.5em}{}[\sectionrule]

% Subsection: sans-serif, no colour
\titleformat{\subsection}
  {\sffamily\large\bfseries}{\thesubsection}{0.5em}{}

% Subsection* (FAQ entries): same but unnumbered
...
\begin{itemize}
    \item \textbf{Draw your schematic in KiCad} and run the
          Electrical Rules Checker (\emph{Inspect $\to$ Electrical Rules Checker}).
          Fix any errors before continuing.

    \item \textbf{Assign footprints.} Use KiCad's footprint assignment tool,
          or let \texttt{pyautoroute-assign} fill them in automatically from a
          personal preference database:
\begin{lstlisting}
pyautoroute-assign MyBoard.kicad_sch
\end{lstlisting}

    \item \textbf{Push the schematic to a PCB.} In KiCad open the PCB editor
          and choose \emph{Tools $\to$ Update PCB from Schematic} (\texttt{F8}).
          Add an \texttt{Edge.Cuts} board outline that generously encloses all
          footprints (at least 2\,mm of margin), then save.

    \item \textbf{Place and route with PyAutoRoute.}
          The single command below places the footprints, then routes all nets:
\begin{lstlisting}
pyautoroute MyBoard.kicad_pcb --place --place-time 60 --routing-time 60
\end{lstlisting}
          A graphical interface is also available (\texttt{pyautoroute-gui})
          if you prefer to watch the optimiser converge in real time or set
          per-footprint constraints (lock a connector, pull a LED to the edge, etc.).

    \item \textbf{Review the result in KiCad.} Open
          \texttt{MyBoard\_placed\_routed.kicad\_pcb}, run
          \emph{Inspect $\to$ Design Rules Checker}, and expect zero
          clearance violations. PyAutoRoute also prints a self-check summary:
\begin{lstlisting}
...
The rest of this tutorial walks through each step in detail using a worked
example --- a selectable-clock board for a Z80 retrocomputer.
...
board after a rectangular outline has been drawn around the components. If you are
intending to use PyAutoRoute to place components, draw a board outline that is generously
sized around the components to give them room to move. You are free to move any components
you want into a specific place and lock them --- PyAutoRoute will honour that. The placer
may not use all the space, but you can easily resize the board once routing and placement
are done if needed. Of course, if you have a specific board size or shape requirement,
draw that outline now and pass \texttt{--keep-outline} (or enable \emph{Keep board outline}
in the GUI Advanced Settings, Section~\ref{sec:advanced}) to constrain placement inside it.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{images/Border.png}
...
\subsection{Letting PyAutoRoute tune itself}
\label{sec:autotune}

The best grid pitch and via weight depend on the board, and guessing them
by hand is tedious. PyAutoRoute can find good values for you in two ways.

\textbf{Offline sweep.} \texttt{pyautoroute-tune} routes the board several
times across a range of settings, scores each result with a single
objective (completion first, then wirelength, then vias, with a small
runtime tiebreaker), and reports the best setting it found:

\begin{lstlisting}
pyautoroute-tune TutorialProject/TutorialProject.kicad_pcb \
    --time 5 --seeds 3
\end{lstlisting}

Here \texttt{--time} is the per-trial routing budget and \texttt{--seeds}
the number of random seeds averaged per setting (annealing is stochastic).
Use \texttt{--time-weight} to trade quality against speed: \texttt{0} ranks
purely on route quality, while a larger value favours coarser, faster grids
when the quality difference is small.

\textbf{Online autotune.} The opt-in \texttt{--auto} flag folds the same
idea into a normal routing run: PyAutoRoute first runs a quick probe on the
board in front of it, picks the best grid and via setting, and (on a
terminal) asks you to confirm before routing the board for real:

\begin{lstlisting}
pyautoroute TutorialProject/TutorialProject.kicad_pcb \
    --auto --routing-time 60
\end{lstlisting}

Pair \texttt{--auto} with \texttt{--write-config} to save the chosen
settings to the project \texttt{.ini} file, so later runs reuse them
without re-probing. Either way, autotune is a convenience for finding a
sensible starting point --- it does not change what the router can achieve,
only how quickly you arrive at good settings.
...
Note that the right-click menu is only available when the canvas is in
\textbf{Initial View} mode --- select it from the view selector at the
bottom of the canvas before right-clicking.
